...
- `scripts/release-pipeline.sh --bundle app --allow-dirty`: 构建 hook sidecar，运行检查，并创建 macOS `.app` bundle。
- `scripts/release-pipeline.sh --bundle dmg --clean-required`: 执行完整 release smoke，包含 DMG 打包和 core 边界字符串检查。
