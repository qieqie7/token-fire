...
#[derive(Clone)]
pub struct CursorHookConfigManager {
    config_path: PathBuf,
    backups_dir: PathBuf,
    #[cfg(debug_assertions)]
...
impl std::fmt::Debug for CursorHookConfigManager {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("CursorHookConfigManager")
...
impl CursorHookConfigManager {
    ...
    pub fn install(&self, hook_path: &Path) -> anyhow::Result<HookInstallResult> {
        ...
        let desired = tokenfire_hook_command(hook_path, TokenSourceKind::Cursor);
    ...
    fn backup(&self, original: &str) -> anyhow::Result<()> {
        ...
        let backup = self
            .backups_dir
            .join(format!("cursor-hooks.json.{timestamp}.{sequence}"));
...
fn remove_tokenfire_hooks(doc: &mut Value) -> bool {
    ...
    let before = stop.len();
    stop.retain(|entry| !is_tokenfire_entry(entry));
    stop.len() != before
}
...
fn existing_tokenfire_commands(doc: &Value) -> Vec<&str> {
    doc.pointer("/hooks/stop")
        ...
        .into_iter()
        .flatten()
        .filter(|entry| is_tokenfire_entry(entry))
...
fn tokenfire_command_from_config(config_path: &Path) -> anyhow::Result<Option<String>> {
    ...
    Ok(doc
        ...
        .into_iter()
        .flatten()
        .find(|entry| is_tokenfire_entry(entry))
        .and_then(|entry| entry.get("command").and_then(Value::as_str))
        .map(str::to_string))
}

fn is_tokenfire_entry(value: &Value) -> bool {
    value
        .get("command")
        .and_then(Value::as_str)
        .is_some_and(|command| {
            is_tokenfire_owned_command_for_source(command, TokenSourceKind::Cursor)
        })
}
